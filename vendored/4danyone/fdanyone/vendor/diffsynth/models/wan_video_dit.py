...
# FDANYONE_ATTENTION overrides the silent installed-package priority so runs
# stay reproducible: auto (default) keeps the policy above, sdpa/sage/flash2/
# flash3 force one implementation and fail loudly when it is unavailable.
import os as _os

_FORCED_ATTENTION = _os.environ.get("FDANYONE_ATTENTION", "auto").strip().lower()
if _FORCED_ATTENTION in ("", "auto"):
    pass
elif _FORCED_ATTENTION == "sdpa":
    FLASH_ATTN_3_AVAILABLE = FLASH_ATTN_2_AVAILABLE = SAGE_ATTN_AVAILABLE = False
elif _FORCED_ATTENTION == "sage":
    if not SAGE_ATTN_AVAILABLE:
        raise RuntimeError("FDANYONE_ATTENTION=sage, but sageattention is not installed.")
    FLASH_ATTN_3_AVAILABLE = FLASH_ATTN_2_AVAILABLE = False
elif _FORCED_ATTENTION in ("flash2", "flash_attn_2"):
    if not FLASH_ATTN_2_AVAILABLE:
        raise RuntimeError("FDANYONE_ATTENTION=flash2, but flash_attn is not installed.")
    FLASH_ATTN_3_AVAILABLE = SAGE_ATTN_AVAILABLE = False
elif _FORCED_ATTENTION in ("flash3", "flash_attn_3"):
    if not FLASH_ATTN_3_AVAILABLE:
        raise RuntimeError("FDANYONE_ATTENTION=flash3, but flash_attn_interface is not installed.")
    FLASH_ATTN_2_AVAILABLE = SAGE_ATTN_AVAILABLE = False
else:
    raise RuntimeError(f"Unknown FDANYONE_ATTENTION value: {_FORCED_ATTENTION!r}")
