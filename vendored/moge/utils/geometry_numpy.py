...
def solve_optimal_shift(uv: np.ndarray, xyz: np.ndarray, focal: float):
    ...
    # A near-empty mask tile (e.g. a uniform sky/ground region of an aerial panorama)
    # can leave too few valid points here; scipy's 'lm' then raises "residuals less
    # than variables" (it needs #residuals >= #variables, and there is 1 variable).
    # There is nothing to fit, so report zero shift and let the empty tile be merged
    # out downstream rather than crashing the whole depth pass.
    if z.size == 0:
    ...
    # Keep 'lm' for every non-empty tile so depth is byte-identical to before for
    # any panorama that already worked (only the empty-tile case above was failing).
