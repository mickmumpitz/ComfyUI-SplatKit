...
def split_panorama_image(image: np.ndarray, extrinsics: np.ndarray, intrinsics: np.ndarray, resolution: Union[int,tuple]):
    height, width = image.shape[:2]
    if isinstance(resolution, int):
        uv = utils3d.numpy.image_uv(width=resolution, height=resolution)
    else:
        uv = utils3d.numpy.image_uv(width=resolution[0], height=resolution[1])
    splitted_images = []
...
def merge_panorama_depth(width: int, height: int, distance_maps: List[np.ndarray], pred_masks: List[np.ndarray], extrinsics: List[np.ndarray], intrinsics: List[np.ndarray]):
    ...
    for i in range(len(distance_maps)):
        ...
        #print(f"\n\nshape chec 1k: {mask_x.shape}\n\n")
        panorama_pred_masks.append(panorama_pred_mask)  
    # print(f"len of grad mask list: {len(panorama_grad_masks)}")
    ...
    # print(f"len of grad panorama_grad_mask_x list: {panorama_grad_mask_x.shape}")
...
# 把泊松融合的事情弄到这块会怎么样？
# some problem related to resolution。
# besides, the seam misalignment.
# quite a lot.
...
@click.option('--pretrained', 'pretrained_model_name_or_path', type=str, default='checkpoints/moge/model.pt', help='Pretrained model name or path. Default is "Ruicheng/moge-vitl"')
...
def main(
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Total images', disable=len(image_paths) <= 1)):
        ...
        # yep， all there.
        splitted_extrinsics, splitted_intriniscs = get_panorama_cameras(fov_x=100.,fov_y=100.)
        splitted_resolution = 768
